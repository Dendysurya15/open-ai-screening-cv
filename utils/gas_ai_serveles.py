...
def ensure_lm_studio_running():
    ...
    try:
        ...
        if "meta-llama-3.1-8b-instruct" not in ps_output.stdout:
            print("Loading model...")
            subprocess.run([
                'lms', 'load',
            ...
            time.sleep(10)  # Give time for model to load
            
        print("LM Studio server ready")
        return True
        
    except Exception as e:
        print(f"Error in ensure_lm_studio_running: {str(e)}")
        return False

def evaluate_candidate(input_data, test_mode=False):
    """Evaluate candidate using the OpenAI API"""
    try:
        # Ensure LM Studio is running before evaluation
        if not ensure_lm_studio_running():
            raise RuntimeError("Failed to start LM Studio server")
        ...
        try:
            ...
            # Tambahkan output schema ke system message
            system_message_with_schema = {
                **system_message,
                "output_schema": {
                    "type": "object",
                    "properties": {
                        "candidates": {
                            ...
                            "items": {
                                "type": "object",
                                "required": ["id_kandidat", "nama_kandidat", "penilaian", "rekomendasi", "ringkasan_penilaian"],
                                "properties": {
                                    "id_kandidat": {"type": "integer"},
                                    "nama_kandidat": {"type": "string"},
                                    "penilaian": {
                                        "type": "array",
                                        "items": {
                                            "type": "object",
                                            "required": ["kategori", "nilai"],
                                            "properties": {
                                                "kategori": {"type": "string"},
                                                "nilai": {"type": "string", "enum": ["1","2","3","4","5"]},
                                                "uraian": {"type": "string"}
                                            }
                                        }
                                    },
                                    "rekomendasi": {"type": "string", "enum": ["Disarankan", "Tidak disarankan"]},
                                    "ringkasan_penilaian": {"type": "string"}
                                }
                            }
                        }
                    }
            ...
            stream = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": json.dumps(system_message_with_schema, ensure_ascii=False)},
